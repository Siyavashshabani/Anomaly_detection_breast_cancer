...
def set_decoder_requires_grad(network, requires_grad):
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            if epoch % 2 == 0:
                ...
                set_decoder_requires_grad(autoenc.model, True)
                print("Training full network...")
            else:
                # Freeze the decoder and train only the encoder and classifier
                set_decoder_requires_grad(autoenc.model, False)
                print("Training encoder and classifier only...")

            # Recreate the optimizer at the start of each epoch
            optimizer = optim.Adam(filter(lambda p: p.requires_grad, autoenc.model.parameters()), lr=0.001)
            train_loss = autoenc.train(epoch, loss_function,optimizer )
